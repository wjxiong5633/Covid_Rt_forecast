...
Fit_functions = list(nn = nn_fit,gam = gam_fit,gam = gam_fit,gbm = gbm_fit)
...
run_model = function(modelname){
  ...
  for(m in 1:length(test_start_period)){
    ...
    for (h in 0:14) {
      ...
      nowcast_model = gam_fit(training.set,vars_final)
...
gam_reportcase_extrapo = run_model("gam")
gam_reportcase_extrapo = read.csv("./pred_reportcase_point/gam_reportcase_point.csv") %>% 
  mutate(date = as.Date(date),
         mytestdate = as.Date(mytestdate))

test = gam_reportcase_extrapo %>% 
