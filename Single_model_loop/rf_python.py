...
# def rf_fit(training_set, vars_final):
...
def rf_predict(model, new_data, vars_final, scaler):
...
def run_model_multipleY(modelname): 
    ...
    for i_date in range(0,len(test_start_period)) :
        ...
        for h in range(0,15):
            ...
            model, scaler = rf_fit(training_set, vars_final)
            ...
            train_pred = rf_predict(model, train_data_x, vars_final, scaler)
            ...
            test_forecast_res = rf_predict(model, test_forecast_data_x, vars_final, scaler) 
...
rf_all = run_model_multipleY("rf")


rf_all = pd.read_csv("./pred_reportcase_point/rf_reportcase_point.csv")
print((rf_all.groupby("proj").apply(lambda group: pd.Series({
            "rmse1": rmse(group["pred_reportcase"], group["true_reportcase"]),
            "rmse2": rmse(group["const_reportcase"], group["true_reportcase"])
            })).reset_index()
            ))
...
print(rmse(rf_all["pred_reportcase"],rf_all["true_reportcase"]))
print(rmse(rf_all["const_reportcase"],rf_all["true_reportcase"]))