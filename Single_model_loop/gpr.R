...
Fit_functions = list(nn = nn_fit,gpr = gpr_fit,gpr = gpr_fit,gbm = gbm_fit,
                     gpr = gpr_fit, gpr = gpr_fit,svr_L = svr_L_fit,gpr = gpr_fit)
...
run_model = function(modelname){
  ...
  for(m in 1:length(test_start_period)){
    ...
    for (h in 0:14) {
      ...
      nowcast_model = gpr_fit(training.set,vars_final)
...
gpr_reportcase_extrapo = run_model("gpr")
gpr_reportcase_extrapo = read.csv("./pred_reportcase_point/gpr_reportcase_point.csv") %>% 
  mutate(date = as.Date(date),
         mytestdate = as.Date(mytestdate))

test = gpr_reportcase_extrapo %>% 
