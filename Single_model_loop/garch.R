...
Fit_functions = list(nn = nn_fit,garch = garch_fit,garch = garch_fit,gbm = gbm_fit,
                     garch = garch_fit, garch = garch_fit,svr_L = svr_L_fit,garch = garch_fit)
...
run_model = function(modelname){
  ...
  for(m in 1:length(test_start_period)){
    ...
    for (h in 0:14) {
      ...
      nowcast_model = garch_fit(training.set,vars_final)
      train_dat = training.set %>% dplyr::select(-y_pred)
      
      nowcast_res = as.numeric(fitted(nowcast_model)[,1])
      ...
      coef_df = cbind(c("mu","ma1",vars_final,"omega","beta1"),coef(nowcast_model),confint(nowcast_model),modelname) %>% as.data.frame()
      names(coef_df) = c("var","coef","CI_lwr","CI_upr","modelname")
      ...
      test_h  = ugarchforecast(nowcast_model,n.ahead = 1,external.forecasts =  list(mregfor=data.matrix(newdata_x)))@forecast$seriesFor %>% as.numeric()
...
garch_reportcase_extrapo = run_model("garch")
garch_reportcase_extrapo = read.csv("./pred_reportcase_point/garch_reportcase_point.csv") %>% 
  mutate(date = as.Date(date),
         mytestdate = as.Date(mytestdate))

test = garch_reportcase_extrapo %>% 
