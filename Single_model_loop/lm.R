...
Fit_functions = list(nn = nn_fit,lm = lm_fit,lm = lm_fit,gbm = gbm_fit,
                     lm = lm_fit, lm = lm_fit,svr_L = svr_L_fit,lm = lm_fit)
  ...
  for(m in 1:length(test_start_period)){
    ...
    for (h in 0:14) {
      ...
      if(modelname  %in% c("lm")){
        ...
        reduced_vars = get_reduced_vars_tillOptimal(C0_select, c("scaled_case","ct_sm_7"),meteorology,pollutant,stringency)
        vars = c(reduced_vars)
      }else{
      ...
      nowcast_model = lm_fit(training.set,vars_final)
      ...
      coef_df = cbind(test_start_date,names(coef(nowcast_model)),coef(nowcast_model),confint(nowcast_model),coef(summary(nowcast_model))[,"Std. Error"],modelname,h) %>% as.data.frame()
      names(coef_df) = c("mytestdate","var","coef","CI_lwr","CI_upr","sd","modelname","proj")
...
lm_reportcase_extrapo = run_model("lm")
lm_reportcase_extrapo = read.csv("./pred_reportcase_point/lm_reportcase_point.csv") %>% 
  mutate(date = as.Date(date),
         mytestdate = as.Date(mytestdate))

test = lm_reportcase_extrapo %>% 
