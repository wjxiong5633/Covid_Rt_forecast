...
Fit_functions = list(nn = nn_fit,lasso = lasso_fit,lasso = lasso_fit,gbm = gbm_fit,
                     lasso = lasso_fit, lasso = lasso_fit,svr_L = svr_L_fit,lasso = lasso_fit)
...
run_model = function(modelname){
  ...
  for(m in 1:length(test_start_period)){
    ...
    for (h in 0:14) {
      ...
      nowcast_model = lasso_fit(training.set,vars_final)
      train_dat = training.set %>% dplyr::select(-y_pred)
      
      nowcast_res =  predict(nowcast_model, s = "lambda.1se", newx = as.matrix(train_dat))%>% as.numeric()
      ...
      coef_df = coef(nowcast_model, s = "lambda.1se")
      coef_matrix <- coef(nowcast_model, s = "lambda.1se") %>% as.matrix()
      non_zero <- coef_matrix[coef_matrix != 0, , drop = FALSE]
      coef_df = data.frame(mytestdate = test_start_date, proj = h,
                           var = rownames(non_zero),coef = non_zero[,1],modelname = modelname)
      ...
      newdata_x = testing_set_date[,vars_final]
      test_h = predict(nowcast_model, s = "lambda.min", newx = as.matrix(newdata_x)) %>% as.numeric()
...
lasso_reportcase_extrapo = run_model("lasso")
lasso_reportcase_extrapo = read.csv("./pred_reportcase_point/lasso_reportcase_point.csv") %>% 
  mutate(date = as.Date(date),
         mytestdate = as.Date(mytestdate))

test = lasso_reportcase_extrapo %>% 
