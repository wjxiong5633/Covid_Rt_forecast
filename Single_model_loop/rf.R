...
Fit_functions = list(nn = nn_fit,rf = rf_fit,rf = rf_fit,gbm = gbm_fit,
                     rf = rf_fit, rf = rf_fit,svr_L = svr_L_fit,rf = rf_fit)
...
run_model = function(modelname){
  ...
  for(m in 1:length(test_start_period)){
    ...
    for (h in 0:14) {
      ...
      nowcast_model = rf_fit(training.set,vars_final)
      ...
      varimp = importance(nowcast_model) %>% as.data.frame()

      coef_df = varimp %>%
        dplyr::rename("coef" = "IncNodePurity")
      coef_df$var = rownames(coef_df)
      coef_df$modelname = modelname
      coef_df$proj = h
      coef_df$mytestdate = test_start_date
    ...
    rmse_df =
      test_reportcase %>%
      group_by(proj) %>%
      dplyr::summarise(rmse1 = rmse(pred_reportcase,true_reportcase),
...
rf_reportcase_extrapo = run_model("rf")
rf_reportcase_extrapo = read.csv("./pred_reportcase_point/rf_reportcase_point.csv") %>% 
  mutate(date = as.Date(date),
         mytestdate = as.Date(mytestdate))

test = rf_reportcase_extrapo %>% 
  mutate(diff = (pred_reportcase-true_reportcase)/true_reportcase) %>% 
  mutate(proj = as.numeric(date - mytestdate)) %>% 
  filter(mytestdate >= "2022-05-28")
