...
Fit_functions = list(nn = nn_fit,arima = arima_fit,arima = arima_fit,gbm = gbm_fit,
                     arima = arima_fit, arima = arima_fit,svr_L = svr_L_fit,arima = arima_fit)
...
run_model = function(modelname){
  ...
  print(modelname)
  test_start_period = seq(as.Date("2022-05-28"),as.Date("2022-12-31"),by = "month")
  res = NULL
  save_train_res = NULL
  coef_all_df = NULL
  m =1
  ...
  for(m in 1:length(test_start_period)){
    ...
    for (h in 0:14) {
      ...
      nowcast_model = arima_fit(training.set,vars_final)
      ...
      coef_df = cbind(test_start_date,names(coef(nowcast_model)),coef(nowcast_model),confint(nowcast_model),modelname,h) %>% as.data.frame()
      names(coef_df) = c("mytestdate","var","coef","CI_lwr","CI_upr","modelname","proj")
      ...
      test_h  = as.numeric(forecast(nowcast_model,xreg = data.matrix(newdata_x))$mean)
...
arima_reportcase_extrapo = run_model("arima")
arima_reportcase_extrapo = read.csv("./pred_reportcase_point/arima_reportcase_point.csv") %>% 
  mutate(date = as.Date(date),
         mytestdate = as.Date(mytestdate))

test = arima_reportcase_extrapo %>% 
