...
Fit_functions = list(nn = nn_fit,svr = svr_fit,svr = svr_fit,gbm = gbm_fit)
...
run_model = function(modelname){
  ...
  for(m in 1:length(test_start_period)){
    ...
    for (h in 0:14) {
      ...
      nowcast_model = svr_fit(training.set,vars_final)
      ...
      coef_df = cbind(names(coef(nowcast_model)),coef(nowcast_model),modelname,h,test_start_date) %>% as.data.frame()
      names(coef_df) = c("var","coef","modelname","proj","mytestdate")
...
svr_reportcase_extrapo = run_model("svr")
svr_reportcase_extrapo = read.csv("./pred_reportcase_point/svr_reportcase_point.csv") %>% 
  mutate(date = as.Date(date),
         mytestdate = as.Date(mytestdate))

test = svr_reportcase_extrapo %>% 
