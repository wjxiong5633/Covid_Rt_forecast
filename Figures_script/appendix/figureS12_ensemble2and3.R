...
interval_ensemble3 <- read_csv("./pred_rt_interval/interval_ensemble3.csv")
interval_ensemble3$modelname = "ensemble3"
write.csv(interval_ensemble3,"./pred_rt_interval/interval_ensemble3.csv", row.names = FALSE )

model_name_list = c("const1","const","arima","gam","garch","gbm","gpr","gru","lm","rf","svr","xgb",'ensemble','ensemble2','ensemble3')
modelname_clean =  c("BASELINE","CONST","ARIMA","GAM","GARCH","GBM","GPR","GRU","LM","RF","SVR","XGB","ENSEMBLE","ENSEMBLE2",'ENSEMBLE3')
...
val.rank
# 
# ggsave(val.rank, file = "./figures/figure_error_avg_rt_ensemble23.pdf",dpi = 300, width = 8, height = 7)
# ggsave(val.rank, file = "./figures_png/figure_error_avg_rt_ensemble23.png",dpi = 300,width = 8, height = 7)
...
pal<-c("black", brewer.pal(10,"Paired"),brewer.pal(6, "Dark2")[6],"turquoise",brewer.pal(6, "Dark2")[3],brewer.pal(6, "Dark2")[4])
...
val_linewidth = c(rep(0.5, 12), c(1,1,1))
val_point_size = c(rep(1.3, 12), c(1.8,1.8,1.8))


val_col = c("BASELINE" = pal[1],"ARIMA" = pal[2],"GAM"= pal[3], "GARCH"  = pal[4],   "GBM" = pal[5],
            "GPR" = pal[6],   "GRU"  = pal[7],    "LM"  = pal[8],    "RF"  = pal[9] ,    "SVR"  = pal[10],
            "XGB" = pal[11],"CONST" = pal[12], "ENSEMBLE" = pal[13] ,"ENSEMBLE2" = pal[14],"ENSEMBLE3" = pal[15])


rt.rank.proj = 
  rt.relative %>% 
  dplyr::group_by(variable,proj) %>%
  dplyr::mutate(relative_index = value/value[modelname== "BASELINE"]) %>% 
  modify_if(~is.numeric(.), ~round(., 3)) %>%
  dplyr::arrange(relative_index) %>% 
  as.data.frame() %>% 
  dplyr::group_by(variable,proj) %>%
...
p_all = ggarrange(val.rank,metrics_value, nrow = 2,labels = c("A","B"))+
    theme( plot.margin = unit(c(0.5,0.5,0.5,0.5), "cm"))

ggsave(p_all, file = "./figures_clean/figure_S11_errors_rt_ensemble23.pdf",dpi = 300,width = 10, height = 10)
# proj.relative <-
#   rt.relative %>%
#   mutate(variable = factor(variable,levels = c("rmse","mape","wis","mae","smape"),
#                            labels = c("RMSE","MAPE","WIS","MAE","SMAPE"))) %>% 
...
# library(ggpubr)
# 
# p_all = ggarrange(metrics_value,proj.relative,as_ggplot(leg), nrow = 3,labels = c("A","B",""), heights = c(1,1,0.2))+
#   theme( plot.margin = unit(c(0.2,0.2,0.2,0.2), "cm"))
# p_all
# 
# ggsave(p_all, file = "./figures/figure_errors_rt_horizon.pdf",dpi = 300, width = 10, height = 10)
# ggsave(p_all, file = "./figures_png/figure_errors_rt_horizon.png",dpi = 300,width = 10, height = 10)
# 
