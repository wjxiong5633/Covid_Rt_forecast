...
for (iDate in seq_along(dates_analysis)) {
  ...
  quant <-
    null_model_forecast_quantiles(dat_train$confirm, horizon = max_prediction_horizon,
                                  truncation = 0)
...
write.csv(res2, './pred_reportcase_interval/interval_const.csv',row.names = F)
...
res3 = res2 %>% dplyr::select(date,mytestdate,proj,pred_reportcase,modelname,true_reportcase) %>% 
  mutate(type = "test")
write.csv(res3, './pred_reportcase_point/const_reportcase_point.csv',row.names = F)
