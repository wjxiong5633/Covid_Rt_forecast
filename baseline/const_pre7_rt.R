...
max_prediction_horizon<-7+remove_last_n
...
for (iDate in seq_along(dates_analysis)) {
  ...
  dat_train<-data_rt %>%
    filter(date_analysis==dates_analysis[iDate]-7)  %>%
    as.data.frame()
...
write.csv(res2, './pred_rt_interval/interval_const_pre7.csv',row.names = F)


res3 = res2 %>% dplyr::select(date,date_analysis,proj,pred_rt,modelname,true_rt)
write.csv(res3, './pred_rt_point/const_pre7_pred_rt_point.csv',row.names = F)
